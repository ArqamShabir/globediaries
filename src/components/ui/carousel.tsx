...
>(({ orientation = 'horizontal', opts, setApi, plugins, className, children, ...props }, ref) => {
  const [carouselRef, api] = useEmblaCarousel(
    { ...(opts as CarouselOptions), axis: orientation === 'horizontal' ? 'x' : 'y' },
    plugins as CarouselPlugin
  )
  ...
  return (
    ...
    >
      <div ref={ref} className={cn('relative overflow-hidden', className)} role="region" aria-roledescription="carousel" {...props}>
        {children}
      </div>
    </CarouselContext.Provider>
  )
...
const CarouselPrevious = React.forwardRef<HTMLButtonElement, React.ComponentProps<typeof Button>>(({ className, variant = 'outline', size = 'icon', ...props }, ref) => {
  ...
  return (
    <Button
      ...
      size={size}
      className={cn(
        'absolute z-20 h-8 w-8 rounded-full',
        orientation === 'horizontal' ? 'left-2 sm:left-3 md:-left-8 top-1/2 -translate-y-1/2' : '-top-12 left-1/2 -translate-x-1/2 rotate-90',
        className
      )}
...
const CarouselNext = React.forwardRef<HTMLButtonElement, React.ComponentProps<typeof Button>>(({ className, variant = 'outline', size = 'icon', ...props }, ref) => {
  ...
  return (
    <Button
      ...
      className={cn(
        'absolute z-20 h-8 w-8 rounded-full',
        orientation === 'horizontal' ? 'right-2 sm:right-3 md:-right-8 top-1/2 -translate-y-1/2' : '-bottom-12 left-1/2 -translate-x-1/2 rotate-90',
        className
      )}
